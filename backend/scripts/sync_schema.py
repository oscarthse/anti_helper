...
def get_gravity_core_json_schema() -> dict:
    ...
    print("\n📦 Processing: libs/gravity_core/schema.py")
...
def get_db_models_json_schema() -> dict:
    ...
    print("\n📦 Processing: backend/app/db/models.py")
    ...
    if isinstance(schema.get("type"), list):
        ...
        for t in schema["type"]:
            if t == "null":
                ...
                types.append(
                    json_type_to_ts({"type": t, **{k: v for k, v in schema.items() if k != "type"}})
                )
        return " | ".join(types)
...
def main() -> int:
    """Main entry point."""
    print("\n🔄 Antigravity Dev - Schema Synchronization")
    print("   Python (Pydantic) → TypeScript\n")
    ...
    if not gravity_schemas and not db_schemas:
        print("\n✗ Failed to generate any schemas")
        return 1
